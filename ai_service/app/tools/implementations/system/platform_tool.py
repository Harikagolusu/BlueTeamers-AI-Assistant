...
from app.tools.domain.schemas.system_schemas import PlatformSchema
...
class PlatformTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(PlatformSchema, request.arguments)
            result = await self._service.get_platform(schema)
