...
from app.tools.domain.schemas.cybersecurity_schemas import HashReputationSchema

@tool(
    name="hash_reputation",
    description="Check hash rep",
    category=ToolCategory.SECURITY
)
class HashReputationTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(HashReputationSchema, request.arguments)
            result = await self._service.check_hash_reputation(schema)
