...
from app.tools.domain.schemas.cybersecurity_schemas import ThreatSchema

@tool(
    name="threat_lookup",
    description="Lookup threat",
...
class ThreatTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(ThreatSchema, request.arguments)
            result = await self._service.lookup_threat(schema)
