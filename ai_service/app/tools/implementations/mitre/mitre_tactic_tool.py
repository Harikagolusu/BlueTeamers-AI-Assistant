...
from app.tools.domain.schemas.mitre_schemas import TacticLookupSchema

@tool(
    name="mitre_tactic",
    description="Lookup MITRE tactic",
    category=ToolCategory.MITRE
)
class MitreTacticTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(TacticLookupSchema, request.arguments)
            result = await self._service.lookup_tactic(schema)
