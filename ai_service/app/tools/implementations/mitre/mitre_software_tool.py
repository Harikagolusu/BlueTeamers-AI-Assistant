...
from app.tools.domain.schemas.mitre_schemas import SoftwareLookupSchema

@tool(
    name="mitre_software",
    description="Lookup MITRE software",
    category=ToolCategory.MITRE
)
class MitreSoftwareTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(SoftwareLookupSchema, request.arguments)
            result = await self._service.lookup_software(schema)
