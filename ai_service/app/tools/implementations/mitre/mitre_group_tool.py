...
from app.tools.domain.schemas.mitre_schemas import GroupLookupSchema

@tool(
    name="mitre_group",
    description="Lookup MITRE group",
    category=ToolCategory.MITRE
)
class MitreGroupTool(BaseTool):
    ...
    async def execute(self, request: ToolRequest) -> ToolResponse:
        try:
            schema = ValidationService.validate_schema(GroupLookupSchema, request.arguments)
            result = await self._service.lookup_group(schema)
