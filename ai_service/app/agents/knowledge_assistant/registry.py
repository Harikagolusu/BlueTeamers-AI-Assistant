...
from app.agents.knowledge_assistant.prompts import get_prompts
try:
...
def register_prompts(prompt_manager: Optional[Any]) -> None:
    ...
    for p in prompts:
        ...
        # Registration Mock - In a real scenario we call prompt_manager.register(p)
        if hasattr(prompt_manager, "register"):
            prompt_manager.register(p)
    
    # Structured Startup Logging
    logger.info(
        f"Knowledge Assistant | Registered | {len(prompts)} prompts | Version {prompts[0].version if prompts else 'N/A'} | Status: Success"
    )

def register_agent(agent_registry: IAgentRegistry, cap_registry: Any = None, prompt_manager: Any = None) -> None:
    """
    Registers the Knowledge Assistant Agent and its capabilities.
    """
    ...
    try:
        ...
        logger.error(f"Failed to register Knowledge Assistant Agent: {e}")
